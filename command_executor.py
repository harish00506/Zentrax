...
try:
    ...
    print("⚠️  pyautogui not available. Some features will be limited.")
    ...
    def _system_info(self, cmd: Dict) -> Tuple[bool, str]:
        ...
        if info_type in ["battery", "general", "all"]:
            ...
            try:
                ...
                if battery_percent:
                    results.append(f"🔋 Battery: {battery_percent}%")
                    ...
                    if status == "2":
                        results.append("⚡ Status: Charging")
                    elif status == "1":
                        results.append("🔌 Status: On Battery")
                else:
                    results.append("🔋 Battery: Unable to detect (desktop PC?)")
            except Exception as e:
                results.append(f"🔋 Battery: Error getting info - {str(e)}")
        
        if info_type in ["time", "datetime", "general", "all"]:
            now = datetime.now()
            results.append(f"🕐 Time: {now.strftime('%I:%M %p')}")
            results.append(f"📅 Date: {now.strftime('%B %d, %Y')}")
        ...
        if info_type in ["memory", "ram", "all"]:
            try:
                ...
                results.append(f"💾 Memory info retrieved")
            except Exception:
                pass
        ...
        if info_type in ["disk", "storage", "all"]:
            try:
                ...
                results.append(f"💿 Disk info retrieved")
            except Exception:
                pass
